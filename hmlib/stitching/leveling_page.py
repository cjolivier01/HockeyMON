...
const auth=location.hash.slice(1); history.replaceState(null,'',location.pathname);
...
function point(event,canvas,width,height){const rect=canvas.getBoundingClientRect();return [Math.max(0,Math.min(width-1,(event.clientX-rect.left)/rect.width*width)),Math.max(0,Math.min(height-1,(event.clientY-rect.top)/rect.height*height))]}
...
$('panorama').onpointerdown=event=>{if(busy||$('auto').checked)return;drag=point(event,$('panorama'),pano.width,pano.height);$('panorama').setPointerCapture(event.pointerId)};
$('panorama').onpointerup=event=>{if(!drag||busy)return;const end=point(event,$('panorama'),pano.width,pano.height);const start=drag;drag=null;if(Math.abs(start[0]-end[0])<3||Math.abs(start[1]-end[1])<3)return;setCrop([Math.min(start[0],end[0])/pano.width,Math.max(start[0],end[0])/pano.width,Math.min(start[1],end[1])/pano.height,Math.max(start[1],end[1])/pano.height])};
