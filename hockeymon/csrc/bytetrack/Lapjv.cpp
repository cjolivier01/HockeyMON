...
double pt_lapjv_simple(
  ...
  if (cost_matrix.dim() != 2 || cost_matrix.scalar_type() != at::kFloat) {
    throw std::runtime_error("Cost matrix must be a 2D float tensor.");
  }

  int n = cost_matrix.size(0);
  int m = cost_matrix.size(1);

  // Optionally extend cost matrix to a square matrix
  at::Tensor cost;
  if (extend_cost && n != m) {
    int s = std::max(n, m);
    cost = at::full({s, s}, std::numeric_limits<float>::infinity(), at::kFloat);
    cost.slice(0, 0, n).slice(1, 0, m).copy_(cost_matrix);
  } else {
    cost = cost_matrix;
  }

  // Initialize solutions
  row_solution = at::full({n}, -1, at::kLong);
  col_solution = at::full({m}, -1, at::kLong);

  // LAPJV implementation (this is a very basic version, full implementation
  // requires more)
  for (int i = 0; i < n; ++i) {
    ...
    for (int j = 0; j < m; ++j) {
      if (cost[i][j].item<float>() < min_val) {
        min_val = cost[i][j].item<float>();
        min_idx = j;
      }
    }

    if (min_val < cost_limit) {
      row_solution[i] = min_idx;
      col_solution[min_idx] = i;
    }
  }

  // Calculate the assignment cost if required
  double total_cost = 0.0;
  if (return_cost) {
    for (int i = 0; i < n; ++i) {
      if (row_solution[i].item<int>() != -1) {
        total_cost += cost[i][row_solution[i].item<int>()].item<float>();
      }
    }
  }
...
double pt_lapjv_full(
    ...
    float cost_limit,
    bool return_cost) {
  auto n = cost_matrix.size(0);
  auto m = cost_matrix.size(1);
  auto max_dim = std::max(n, m);

  // If extending the cost matrix to square it
  at::Tensor cost;
  if (extend_cost && n != m) {
    cost = at::full(
        {max_dim, max_dim},
  ...
  x = at::full({max_dim}, -1, at::kLong);
  y = at::full({max_dim}, -1, at::kLong);
  at::Tensor min_in_rows = std::get<0>(at::min(cost, 1, true));
  cost -= min_in_rows;

  for (int i = 0; i < max_dim; i++) {
    for (int j = 0; j < max_dim; j++) {
      if (cost[i][j].item<float>() == 0 && y[j].item<int>() == -1) {
        x[i] = j;
        y[j] = i;
        break;
      }
  ...
  // Cover columns and rows, scan for free rows
  at::Tensor covered_cols = at::zeros({max_dim}, at::kByte);
  at::Tensor covered_rows = at::zeros({max_dim}, at::kByte);
  ...
  while (path_found) {
    path_found = false;
    for (int i = 0; i < max_dim; i++) {
      if (x[i].item<int>() == -1) { // Free row
        for (int j = 0; j < max_dim; j++) {
          if (cost[i][j].item<float>() == 0 && !covered_cols[j].item<bool>()) {
            // Augmenting path algorithm needs to be implemented here to update
            // x and y
            covered_rows[i] = 1;
            if (y[j].item<int>() == -1) {
              x[i] = j;
  ...
  // Calculate the cost if needed
  double total_cost = 0.0;
  if (return_cost) {
    for (int i = 0; i < n; i++) {
      if (x[i].item<int>() != -1) {
        total_cost += cost_matrix[i][x[i].item<int>()].item<float>();
      }
    }
  }
