  ...
  for (int n = 0; n < n_levels; ++n) {
    ...
    levels.push_back(
        {width,
         ...
         bytes,
         data,
         std::shared_ptr<MapAlloc::MapAllocEntry>{nullptr},
         x,
         y,
         x_shift,
         y_shift,
         n ? levels[n - 1].x_shift : false,
  ...
  if (step > 1) {
    ...
    for (int t = 0; t < (int)levels[0].bands.size() - 1; ++t) {
      switch (bits) {
        case 8:
          threadpool.Schedule([=] {
            CopyInterleavedThread_8bit(
                src_p, step, pitch, levels[0].bands[t], levels[0].bands[t + 1]);
          });
          break;
        case 16:
          threadpool.Schedule([=] {
            CopyInterleavedThread_16bit(
                (uint16_t*)src_p,
                step,
                pitch,
                levels[0].bands[t],
...
void Pyramid::CopyInterleavedThread_8bit(
    uint8_t* src_p,
    int step,
    int pitch,
    int sy,
...
void Pyramid::CopyInterleavedThread_16bit(
    uint16_t* src_p,
    int step,
    int pitch,
    int sy,
...
void Pyramid::CopyPlanarThread_8bit(
    uint8_t* src_p,
    int pitch,
    bool gamma,
    int sy,
  ...
  for (y = sy; y < ey; ++y) {
    ...
    if (gamma) {
      ...
      for (x = 0; x < sixteens; ++x) {
        pixels = _mm_loadu_si128(src_pp_m++);
        _mm_store_ps(
            (float*)p_p++, _mm_cvtepi32_ps(_mm_shuffle_epi8(pixels, shuffle)));
...
void Pyramid::CopyPlanarThread_16bit(
    uint16_t* src_p,
    int pitch,
    bool gamma,
    int sy,
  ...
  for (y = sy; y < ey; ++y) {
    ...
    if (gamma) {
      ...
      for (x = 0; x < eights; ++x) {
        pixels = _mm_loadu_si128(src_pp_m++);
        _mm_store_ps(
            (float*)p_p++, _mm_cvtepi32_ps(_mm_shuffle_epi8(pixels, shuffle1)));

        _mm_store_ps(
            (float*)p_p++, _mm_cvtepi32_ps(_mm_shuffle_epi8(pixels, shuffle2)));
      }
    }
...
void Pyramid::CopyPlanarThread_32bit(
    __m128* src_p,
    int pitch,
    bool gamma,
    int sy,
    ...
    for (int t = 0; t < (int)levels[l + 1].bands.size() - 1; ++t) {
      threadpool.Schedule([=] {
        ShrinkThread(
            ...
            lo,
            levels[l].m128_pitch,
            levels[l + 1].m128_pitch,
            first_bad_line,
            height_odd,
            levels[l + 1].bands[t],
            levels[l + 1].bands[t + 1],
            levels[l].x_shift,
            levels[l].y_shift);
      });
    }
...
void Pyramid::ShrinkThread(
  ...
  if (sy == 0) {
      ...
      for (x = 0; x < m128_pitch_hi; ++x) {
        _mm_store_ps(
            ...
            _mm_add_ps(
                _mm_add_ps(
                    _mm_add_ps(
                        hi[x - m128_pitch_hi], hi[x + (m128_pitch_hi << 1)]),
                    _mm_mul_ps(
                        _mm_add_ps(
                            hi[x - m128_pitch_hi], hi[x + m128_pitch_hi]),
                        four)),
                _mm_mul_ps(hi[x], six)));
      }
      Squeeze(line, lo, m128_pitch_lo, m128_pitch_hi, _256th, x_shift);
    ...
    hi += m128_pitch_hi *
        (((sy - 1) << 1) -
         y_shift); // because hi has a missing line compared to lo, lo's line 0
                   // corresponds to hi's line -2, 1=>0, 2=>2, 3=>4
  }
...
void Pyramid::LaplaceExpandShifted(
    __m128* hi,
    __m128* lo,
    int m128_pitch_hi,
    int m128_pitch_lo) {
...
void Pyramid::LaplaceLine3(
  ...
  for (int x = 0; x < m128_pitch; ++x) {
    _mm_store_ps(
        ...
        _mm_sub_ps(
            _mm_add_ps(
                _mm_mul_ps(_mm_add_ps(temp1[x], temp3[x]), eighth),
...
void Pyramid::LaplaceThreadWrapper(
    Level* upper_level,
    Level* lower_level,
    int sy,
    int ey) {
  ...
  for (i = 0; i < lim; ++i) {
    ...
    for (y = 0; y < levels[i].height; ++y) {
      for (x = 0; x < levels[i].m128_pitch; ++x) {
        _mm_store_ps(
            (float*)&data[x], _mm_add_ps(__add, _mm_mul_ps(data[x], __mul)));
      }
...
void Pyramid::Multiply(int level, float mul) {
  ...
  for (y = 0; y < levels[level].height; ++y) {
    for (x = 0; x < levels[level].m128_pitch; ++x) {
      _mm_store_ps(
          (float*)&data[x], _mm_mul_ps(_mm_load_ps((float*)&data[x]), __mul));
    }
    data += levels[level].m128_pitch;
  }
}
    ...
    for (y = 0; y < levels[l].height; ++y) {
      for (x = 0; x < levels[l].m128_pitch; ++x) {
        _mm_store_ps(
            (float*)&data[x],
            _mm_mul_ps(
                _mm_load_ps((float*)&data[x]), _mm_load_ps((float*)&_b[x])));
      }
      data += levels[l].m128_pitch;
...
void Pyramid::Fuse(
  ...
  for (l = 0; l < (int)levels.size(); ++l) {
    ...
    for (int t = 0; t < (int)levels[l].bands.size() - 1; ++t) {
      threadpool.Schedule([=] {
        FuseThread(
            (__m128*)levels[l].data,
            (__m128*)_b->levels[l].data,
            (__m128*)mask->levels[l].data,
            levels[l].m128_pitch,
            levels[l].bands[t],
            levels[l].bands[t + 1],
            pre,
      ...
      for (p = 0; p < count; ++p) {
        _mm_store_ps(
            ...
            _mm_add_ps(
                blacks,
                _mm_add_ps(
                    _mm_sub_ps(b[p], blacks),
                    _mm_mul_ps(
                        _mm_sub_ps(a[p], blacks), _mm_sub_ps(ones, m[p])))));
      }
    } else {
      for (p = 0; p < count; ++p) {
        _mm_store_ps(
...
void Pyramid::Denoise(int level, float power, bool gamma) {
  ...
  if (gamma)
    _power = _mm_mul_ps(_power, _power);
...
void Pyramid::Blend(Pyramid* b) {
  if (b->GetNLevels() < GetNLevels())
    return;
  memcpy(
      levels[GetNLevels() - 1].data,
      b->levels[GetNLevels() - 1].data,
...
void Pyramid::BlurX(float radius, Pyramid* transpose) {
  ...
  for (int i = 0; i < (int)levels[0].bands.size() - 1; ++i) {
    threadpool.Schedule([=] {
      BlurXThread(
          radius, transpose, levels[0].bands[i], levels[0].bands[i + 1]);
    });
  }
  threadpool.join_all();
}
...
void Pyramid::Png(const char* filename) {
  ...
  Pnger::Quick(
      (char*)filename, temp, width, height, width, PNG_COLOR_TYPE_GRAY);
