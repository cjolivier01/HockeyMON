...
void ReadSeamDT(Flex* flex, int& current_count, std::int64_t& current_step, uint64_t& dt_val) {
  if (current_count) {
    --current_count;
    dt_val += current_step;
  } else {
    ...
    if (_byte == 255) {
      ...
      current_step = ((std::int64_t)(_byte & 7) - 3) << 32;
      if (!(_byte & 0x80)) { // 0b0ccccsss
        current_count = _byte >> 3;
      } else if (!(_byte & 0x40)) { // 0b10ssssss
        current_step = (std::int64_t)(_byte & 0x3f) << 32;
        current_count = 0;
      } else if (!(_byte & 0x20)) { // 0b11000000
        current_count = flex->ReadBackwards8();
      } else if (!(_byte & 0x10)) { // 0b11100000
...
int CompressSeamLine(std::uint64_t* input, std::uint8_t* output, int width) {
  int current_step = -100;
  int current_count = 0;
  ...
  while (x > 0) {
    ...
    if (!((right_val ^ left_val) & 0xffffffff) && (step = ((std::int64_t)(right_val - left_val) >> 32) + 3) < 67 && step >= 0) { // was <= 7
      if (step <= 7) {
        if (step == current_step) {
          ++current_count;
        } else {
...
void SwapUnswapV(Pyramid* py, bool unswap) {
  ...
  if (height & 1) {
    ...
    if (unswap) {
      uint8_t* upper = (uint8_t*)(py->GetData() + ((std::int64_t)height >> 1) * py->GetPitch());
      uint8_t* lower = (uint8_t*)(py->GetData() + ((std::int64_t)height - 1) * py->GetPitch());
    ...
    uint8_t* upper = (uint8_t*)py->GetData();
    uint8_t* lower = (uint8_t*)(py->GetData() + (std::int64_t)half_height * py->GetPitch());
    for (int y = 0; y < half_height; ++y) {
      memcpy(temp, upper, byte_pitch);
