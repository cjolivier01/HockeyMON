"""HockeyMON Python core re‑exports.
...
`hockeymon._hockeymon` (pybind11 + Torch).  All heavy‑lifting (image
...
If you need details not covered here, inspect `hockeymon/src/PythonBindings.cpp`.
"""
...
def _preload_torch_shared_libraries() -> None:
    """
    Ensure Torch's shared libraries are loaded before importing `hockeymon._hockeymon`.
...
from . import _hockeymon as _native_hockeymon
from ._hockeymon import (
    AllLivingBoxConfig,
    AspenGraphSampler,
    BBox,
    BlenderConfig,
...
try:
    from ._hockeymon import EnBlender
except Exception:
    EnBlender = None
...
_doc(
    ...
    placements, and uint16 inverse X/Y maps compatible with HockeyMON's
    stitching remappers.
    """,
)
...
_doc(
    HmByteTracker,
    """BYTETracker variant integrated with HockeyMON pipeline (player detection).""",
)
_doc(
    HmByteTrackerCuda,
